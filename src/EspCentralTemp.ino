...
*/
  ...
  server.on(rota + "/chuveiro", HTTP_ANY, []() {
    addCORS();
  });
  ...
  server.on(rota + "/temp", handleChuveiro);
...
void handleChuveiro() {                         // If a POST request is made to URI /login
  ...
  if(server.method() == HTTP_POST){
    if ( ! server.hasArg("temp_escolhida") || ! server.hasArg("id_banho") || ! server.hasArg("id_perfil")
        ||  server.arg("temp_escolhida") == NULL || server.arg("id_banho") == NULL || server.arg("id_perfil") == NULL )
        {
        ...
        if(server.arg("temp_escolhida") != NULL &&
                 server.arg("id_banho") != NULL && 
                 server.arg("id_perfil") != NULL){
                   
                       server.send(200, "text/plain", "Recebido informações via POST");
  ...
  json["temperatura_ambiente"] = temp_ambiente;
  json["temperatura_final"] = temp_final;
  json["tempo"] = tempo_banho;
